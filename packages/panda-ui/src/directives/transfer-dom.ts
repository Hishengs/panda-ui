 ...
 */
function getTarget(node: Node | boolean | string): Node | null {
  if (node === void 0) {
    node = document.body;
  }
  if (node === true) {
  ...
  return node instanceof window.Node
    ? node
    : document.querySelector(node as string);
}

type El = Element & {
  dataset: any;
  __transferDomData: any;
};

const directive = {
  inserted(el: El, { value }: any, vnode: any) {
  ...
  componentUpdated(el: El, { value }: any) {
    if (el.dataset && el.dataset.transfer !== 'true') return false;
